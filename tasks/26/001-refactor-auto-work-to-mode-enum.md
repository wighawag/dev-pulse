...
- All existing tests in `test/auto-work.test.ts` are updated to use the new mode enum
...
- `src/auto-work.ts`: Refactor `isAutoWorkEnabled()` to switch on mode. For `ai` mode, just fall through to `triggered` logic for now (AI call added in next task).
...
- `src/orchestrator.ts`: Update the `run()` log output to show the auto-work mode instead of boolean.
- `test/auto-work.test.ts`: Update `makeConfig` default and all test cases to use mode strings.
